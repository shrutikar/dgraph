...
func process(dg *dgo.Dgraph, conf *viper.Viper) (Counter, error) {
	...
	counter.mLatency = mutationLatency
	return counter, nil
}
...
retryConn:
	...
	for num > 0 {
		...
		fmt.Println(time.Duration(qLatency).Round(time.Millisecond),
			time.Duration(mLatency).Round(time.Millisecond),
			time.Duration(qLatency+mLatency).Round(time.Millisecond), "]")
		num--
		time.Sleep(waitDur)
	}
}
